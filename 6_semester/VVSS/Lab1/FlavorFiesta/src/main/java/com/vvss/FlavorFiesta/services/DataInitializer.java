...
public class DataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (recipeCount == 0) {
            ...
            List<Recipe> ropes = Arrays.asList(
            ...
            ropes.forEach(recipeService::saveRecipe);

            List<Comment> comments = Arrays.asList(
                new Comment(ropes.get(0), users.get(0), "Not a great recipe"),
                new Comment(ropes.get(1), users.get(0), "Very tasty!"),
                new Comment(ropes.get(1), users.get(1), "Love it!")
            );
